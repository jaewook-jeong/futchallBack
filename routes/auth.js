...
router.get('/myinfo', passport.authenticate('refresh-jwt', { session: false }), async (req, res, next) => {
  try {
    ...
    if (req.headers.authorization.slice(7) !== fullUserWithoutPwd.token) {
      ...
      console.log('------------------------------------');
      return res.cookie('RefreshToken', null, { maxAge: 0, httpOnly: true, domain: process.env.NODE_ENV === 'production' && '.futchall.com', secure: true }).status(403).send("CSRF attack");
    }
